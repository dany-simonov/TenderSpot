...
export async function saveSession(session) {
    const serialized = Array.from(session.cookieJar.entries()).map(([name, value]) => `${name}=${value}`);
    await stateStore.saveCookies(serialized);
}
export async function humanFetch(session, url, referer) {
    await humanPause();
...
function extractRawPriceFragment(input) {
    ...
    const ultimateMatchFromText = blockText.match(/([\d\s\u00A0&nbsp;]{3,},\d{2})\s*(?:₽|руб|Российский рубль)/i);
    if (ultimateMatchFromText?.[0]) {
        return ultimateMatchFromText[0];
    }
    const blockTextWithEntities = normalizeWhitespace(input.replace(/<[^>]+>/g, ' '));
    const ultimateMatchFromRaw = blockTextWithEntities.match(/([\d\s\u00A0&nbsp;]{3,},\d{2})\s*(?:₽|руб|Российский рубль)/i);
...
function buildTenderFromBlock(block, log) {
    const $ = load(block);
    const hrefCandidates = $('a[href]')
        .map((_i, el) => String($(el).attr('href') || '').trim())
        .get()
        .filter(Boolean);
    const preferredHref = hrefCandidates.find((href) => /notice223\/common-info\.html/i.test(href)) ||
        hrefCandidates.find((href) => /regNumber=\d{8,}/i.test(href)) ||
        hrefCandidates[0] ||
        '';
    const externalId = parseRegNumberFromHref(preferredHref) ||
    ...
    const titleRaw = block.match(/registry-entry__header-mid__title[\s\S]*?<a[^>]*>([\s\S]*?)<\/a>/i)?.[1] ??
...
function parseStage2PageData(html) {
    ...
    const legalAddress = extractAddressField(fullText, /(?:место\s+нахождения|юридичес(?:кий|кого)\s+адрес)\s*[:\-]?\s*([\s\S]{0,260}?)(?=(?:почтов(?:ый|ого)\s+адрес|место\s+поставки|место\s+выполнения\s+работ|место\s+оказания\s+услуг|адрес\s+объекта|инн|кпп|огрн|телефон|e-?mail|$))/i);
    const postalAddress = extractAddressField(fullText, /(?:почтов(?:ый|ого)\s+адрес|адрес\s+для\s+корреспонденции)\s*[:\-]?\s*([\s\S]{0,260}?)(?=(?:место\s+нахождения|место\s+поставки|место\s+выполнения\s+работ|место\s+оказания\s+услуг|адрес\s+объекта|инн|кпп|огрн|телефон|e-?mail|$))/i);
    ...
    let deliveryPlace = extractAddressField(fullText, new RegExp(`${deliveryLabelPattern.source}\\s*[:\\-]?\\s*([\\s\\S]{0,320}?)(?=(?:место\\s+нахождения|юридичес(?:кий|кого)\\s+адрес|почтов(?:ый|ого)\\s+адрес|инн|кпп|огрн|телефон|e-?mail|$))`, 'i'));
    if (!deliveryPlace) {
        deliveryPlace = extractFieldFromHtml(html, deliveryLabelPattern);
    }
    const addressContext = normalizeWhitespace(`${legalAddress} ${postalAddress} ${deliveryPlace}`);
    const zipCodes = Array.from(addressContext.matchAll(/\b(\d{6})\b/g)).map((match) => match[1]);
    const validZipCodes = Array.from(new Set(zipCodes.filter((zip) => /^[1-9]\d{5}$/.test(zip))));
    const deliveryZipCodes = Array.from(new Set(Array.from(deliveryPlace.matchAll(/\b(\d{6})\b/g)).map((match) => match[1]))).filter((zip) => /^[1-9]\d{5}$/.test(zip));
...
function isStage2Residential(text) {
    return [' мкд', 'многоквартирн', 'жилой дом', 'жилкомсервис'].some((marker) => normalize(text).includes(normalize(marker)));
}
...
async function browseResultsHumanLike(args) {
    ...
    log(`[stage-1] session started; target duration ${Math.round(targetDuration / 1000)} sec, keywords=${args.keywords.join(', ')}`);
    try {
        ...
        const stage1Deduped = Array.from(stage1Candidates.reduce((acc, tender) => {
        ...
        log(`[stage-1] done: scannedBlocks=${stage1Stats.scannedBlocks}, passed=${stage1Deduped.length}, rejected=${JSON.stringify(stage1Stats.rejectedByReason)}`);
        const stage2Stats = {
            passed: 0,
            rejectedByReason: {},
        };
        ...
        for (const candidate of stage1Deduped) {
            ...
            try {
                ...
                if (!pageData.inn) {
                    ...
                    log(`[stage-2] inn-diagnostics: ${candidate.externalId} | inn-empty | snippet=${JSON.stringify(innSnippet)}`);
                }
                const innPrefix = pageData.inn.slice(0, 2);
                const hasBadInnPrefix = Boolean(innPrefix) &&
                ...
                const hasTargetDeliveryZip = pageData.deliveryZipCodes.length > 0 &&
                    hasTargetZipPrefix(pageData.deliveryZipCodes);
                if (pageData.deliveryPlace ||
                    ...
                    log(`[stage-2] address-diagnostics: ${candidate.externalId} | delivery=${JSON.stringify(deliverySnippet)} | delivery_zips=${JSON.stringify(pageData.deliveryZipCodes)} | all_zips=${JSON.stringify(pageData.validZipCodes)} | delivery_target=${hasTargetDeliveryZip}`);
                }
                if (!hasTargetDeliveryZip &&
                    pageData.validZipCodes.length > 0 &&
                    hasForeignZipPrefix(pageData.validZipCodes) &&
                    !hasTargetZipPrefix(pageData.validZipCodes)) {
                    log(`[stage-2] reject stage2-bad-zip: ${candidate.externalId} | found_zips=${JSON.stringify(pageData.validZipCodes)}`);
        ...
        log(`[etl] Stage 2 summary: passed=${stage2Stats.passed}, rejected=${JSON.stringify(stage2Stats.rejectedByReason)}`);
        log(`[etl] Pipeline summary: scannedBlocks=${stage1Stats.scannedBlocks}, passedStage1=${stage1Deduped.length}, passedStage2=${stage2Enriched.length}. Loaded to DB: ${stage2Enriched.length}.`);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
function hasForeignCity(fullText) {
    ...
    for (const match of matches) {
        ...
        const isAllowed = allowedCities.some((candidate) => city.includes(candidate) || candidate.includes(city));
        if (!isAllowed) {
            return true;
        }
    }
...
function calculateRegionalScore(entry) {
    ...
    const fullText = normalizeCyrillic(`${entry.title} ${entry.customer} ${entry.description} ${entry.lawRaw} ${entry.regionText}`);
    const hasPositiveContext = POSITIVE_REGION_MARKERS.some((marker) => fullText.includes(normalizeCyrillic(marker)));
    const hasDirectCityHit = MOSCOW_MARKERS.some((marker) => fullText.includes(normalizeCyrillic(marker)));
...
function getRejectReason(entry, fileDate, log) {
...
function finalizeTender(entry, fileDate, log) {
...
export async function extractAndFilterEis(args) {
    ...
    const result = rawRows
        ...
        .sort((a, b) => b.relevanceScore - a.relevanceScore)
        .slice(0, MAX_TENDERS);
    args.log?.(`[eis.extract] post-filter result: input=${rawRows.length}, output=${result.length}, mode=${args.method}`);
    return result;
}
