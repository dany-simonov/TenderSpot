...
import { extractAndFilterEis, humanFetch, loadSession, saveSession, } from '../adapters/eis/eis.extract';
...
function findByRegex(lines, patterns) {
    const found = [];
...
function findLabelValues(lines, labels, maxLen = 320) {
    const found = [];
    for (const line of lines) {
...
function parseCardData(html) {
    ...
    const geographyAndContactBlocks = uniqueNonEmpty(lines.filter((line) => /(адрес|телефон|e-mail|email|контакт|индекс|область|район|город|ул\.|улица|дом\b|корпус|строение)/iu.test(line))).slice(0, 40);
    return {
        customerInn: {
...
function toCardRecord(tender, html) {
    return {
...
run().catch((error) => {
    console.error(`[debug-discovery] failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
});
