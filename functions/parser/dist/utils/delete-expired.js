...
async function deleteExpiredTenders() {
    ...
    let deleted = 0;
    while (true) {
        const page = await databases.listDocuments(parserEnv.APPWRITE_DATABASE_ID, parserEnv.APPWRITE_TENDERS_COLLECTION_ID, [Query.lessThan('deadline', cutoffIso), Query.limit(PAGE_LIMIT), Query.orderAsc('$id')]);
        if (page.documents.length === 0) {
            break;
        }
        for (const doc of page.documents) {
            await databases.deleteDocument(parserEnv.APPWRITE_DATABASE_ID, parserEnv.APPWRITE_TENDERS_COLLECTION_ID, doc.$id);
            deleted += 1;
            if (deleted % 20 === 0) {
