...
export class AppwriteTenderLoader {
    ...
    async upsertOne(tender) {
        ...
        try {
            await this.databases.createDocument(this.databaseId, this.collectionId, ID.custom(documentId), createPayload);
        }
        catch (error) {
            const code = error instanceof AppwriteException
                ? error.code
            ...
            await this.databases.updateDocument(this.databaseId, this.collectionId, documentId, updatePayload);
        }
    }
}
